...
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# +++ NEW: SWAP FILE CREATION TO PREVENT OUT-OF-MEMORY ERRORS ON LOW-RAM VPS +++
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# +++ END OF NEW SWAP FILE SECTION +++
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

PACKAGES_TO_INSTALL=(
    "wireguard"      # For wg, wg-quick, and kernel module
    "qrencode"       # For QR codes
    "curl"           # For public IP fetching
    "git"            # For cloning blind-operator-mode
    "make"           # For building blind-operator-mode
    "build-essential" # For general build tools
    "linux-headers-$(uname -r)" # For DKMS kernel modules
    "dkms"           # For blind-operator-mode kernel module
    "ufw"            # For firewall management
    "software-properties-common" # General utility, sometimes for PPAs
)
...
sed -i '/^#net.ipv4.ip_forward=1/s/^#//' /etc/sysctl.conf # Uncomment if present
sysctl -p

# Configure UFW (Firewall)
ufw allow ${SERVER_WG_PORT}/udp
ufw allow ssh # Keep SSH open for now, will be removed later
echo "y" | ufw enable # Enable UFW, non-interactively (alternative to --force)
...
# Extract base IP for clients (e.g., 10.0.100 from 10.0.100.1)
BASE_CLIENT_IP=$(echo "${SERVER_WG_IPV4}" | awk -F. '{print $1"."$2"."$3}')
CLIENT_IP_START_OCTET=2 # Server is .1
...
# Restart WireGuard to apply new peers
...
echo -e "${YELLOW}# explicitly states this is 'mostly snake-oil' and a 'toy'. [1]                   #"
echo -e "${YELLOW}# He advises: 'Do not use this code unless you fully understand what it is        #"
echo -e "${YELLOW}# not designed to do... In fact, just don't use it.' [1]                          #"
echo -e "${YELLOW}# There are many ways to subvert this. [1]                                        #"
echo -e "${YELLOW}# PROCEED AT YOUR OWN RISK. This script is for informational purposes only.       #"
echo -e "${YELLOW}###################################################################################${NC}"

# Kernel requirements check (conceptual)
echo "[INFO] Checking for kernel configuration requirements (CONFIG_SECURITY, CONFIG_SECURITY_NETWORK, CONFIG_KALLSYMS_ALL)..."
CONFIG_FILE="/boot/config-$(uname -r)"
if [ -f "$CONFIG_FILE" ]; then
    echo "[INFO] Contents of $CONFIG_FILE related to BOM requirements:"
    grep -E "CONFIG_SECURITY=y|CONFIG_SECURITY_NETWORK=y|CONFIG_KALLSYMS_ALL=y" "$CONFIG_FILE" || echo "[INFO] Some BOM-related kernel configs not found or not set to 'y' in $CONFIG_FILE."
else
    echo "[WARNING] Kernel config file $CONFIG_FILE not found. Cannot automatically verify all kernel options."
fi
echo "[WARNING] Manual verification of kernel options CONFIG_SECURITY, CONFIG_SECURITY_NETWORK, and CONFIG_KALLSYMS_ALL is still recommended. [1]"

# Clone the repository
REPO_URL="https://git.zx2c4.com/blind-operator-mode"
REPO_DIR="blind-operator-mode"
BOM_INSTALL_PATH="/opt/${REPO_DIR}" # Install to /opt to keep /root clean

echo "[INFO] Cloning the blind-operator-mode repository from $REPO_URL to ${BOM_INSTALL_PATH}..."
...
if ! git clone "$REPO_URL" "$BOM_INSTALL_PATH"; then
  echo -e "${RED}[ERROR] Failed to clone the blind-operator-mode repository. Please check the URL and your internet connection.${NC}"
  exit 1 # Critical failure if BOM can't be fetched
fi

cd "$BOM_INSTALL_PATH"

# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# +++ NEW: KERNEL COMPATIBILITY PATCH FOR blind-operator-mode +++
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# This patch fixes a compilation error on Kernel 5.4+ due to a change in
# how LSM hooks are handled (from list_head to hlist_node).
# We change the function call from list_add_tail_rcu to its hash-list equivalent.
...
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# +++ END OF NEW KERNEL PATCH SECTION +++
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

# Installation as per the README [1]
echo "[INFO] Attempting to install blind-operator-mode using 'make install' (requires DKMS)..."
if make install; then
  echo -e "${GREEN}[INFO] 'make install' completed for blind-operator-mode. The module should be handled by DKMS.${NC}"
  echo "[INFO] The blind operator mode module, if successfully built and installed via DKMS,"
  echo "       will likely load on boot. Its effects, such as disabling raw sockets,"
  echo "       may be delayed (e.g., 60 seconds after loading) to allow services like DHCP to start. [1]"
  
  echo "[INFO] Configuring ping to work with disabled raw sockets (as per blind-operator-mode README)..."
  if ! grep -q "^net.ipv4.ping_group_range=\"0 0\"" /etc/sysctl.conf && ! grep -q "^net.ipv4.ping_group_range=0 0" /etc/sysctl.conf ; then
      echo "net.ipv4.ping_group_range=\"0 0\"" >> /etc/sysctl.conf
  fi
  sysctl -w net.ipv4.ping_group_range="0 0" # Allow non-root ping
  sysctl -p # Apply sysctl changes
else
  echo -e "${RED}[ERROR] 'make install' for blind-operator-mode failed. Please check the output for errors.${NC}"
  echo "        Ensure you have DKMS, kernel headers, and build tools correctly installed (Phase 1 tried this)."
  echo "        Also, verify that your kernel meets the requirements mentioned in the README. [1]"
  cd - > /dev/null # Go back to previous directory
  # Not exiting, as user might want to proceed with lockout anyway, but BOM won't be active.
  # User confirmed they want to proceed despite risks.
fi
cd - > /dev/null # Go back to previous directory
...
# Before purging, ensure apt doesn't get stuck on prompts if any interactive config removal happens
...
systemctl mask getty@tty1.service || echo "[WARN] getty@tty1 mask failed." # Prevents manual start

# Attempt to disable other potential console services
